...
def set_snapshot_points(points):
    global _snapshot_points
    _snapshot_points = points
...
def save_itr_params(itr, params_dict):
    ...
    if _snapshot_dir:
        ...
        if _snapshot_mode == 'all':
            ...
            file_names = [osp.join(_snapshot_dir, 'weights', n + '_itr_%d.pth' % itr) for n in names]
            save_weights(params, file_names)
        elif _snapshot_mode == 'last':
            # override previous params
            file_names = [osp.join(_snapshot_dir, 'weights', n + '.pth') for n in names]
            save_weights(params, file_names)
            ...
            file_names = [osp.join(_snapshot_dir, 'weights', n + '.pth') for n in names]
            save_weights(params, file_names)
        elif _snapshot_mode == "specific":
            if itr in _snapshot_points:
                file_names = [osp.join(_snapshot_dir, 'weights', n + '_itr_%d.pth' % itr) for n in names]
                save_weights(params, file_names)
